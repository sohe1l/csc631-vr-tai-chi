...
public class VisualVC : MonoBehaviour
{
    ...
    void Update()
    {
        try
        {
            ...
            if (delta > 0.1)
            {
                ...
                //Debug.Log(RealHandLeft.transform.position);

                //first input is database value, second input is player input value
                //DrawVisual(TargetHandLeft.transform.position, RealHandLeft.transform.position);
                //DrawVisual(RealHandLeft.transform.position, RealHandLeft.transform.position);

                StartCoroutine(DrawVisual(TargetHandLeft.transform.position, RealHandLeft.transform.position));
                StartCoroutine(DrawVisual(TargetHandRight.transform.position, RealHandRight.transform.position));
    ...
    //first input is database value, second input is player input value
    IEnumerator DrawVisual(Vector3 real, Vector3 trial)
    {
        bool acceptable = validatePoint(real, trial);

        GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        sphere.transform.position = real;
        sphere.transform.localScale = new Vector3((float).1, (float).1, (float).1);
